...
def webhook():
    ...
    if s["state"] in (S_WELCOME,S_WAITING_ORDER):
        if is_image:
            send_text(phone,"Por favor, preciso do numero do pedido antes das imagens."); s["state"]=S_WAITING_ORDER
        elif text and looks_like_order(text):
            order=extract_order(text); s["order"]=order; s["state"]=S_WAITING_IMAGES
            send_text(phone,f"Numero do pedido *{order}* recebido! Agora pode enviar as suas imagens.")
        elif s["state"]==S_WELCOME:
            send_text(phone,"Olá, seja bem vindo!!! Antes de enviar as suas imagens preciso que você me envie o número do pedido.")
            s["state"]=S_WAITING_ORDER
        else:
            send_text(phone,"Por favor, me envie o número do pedido para continuar.")
