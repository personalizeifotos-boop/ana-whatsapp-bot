...
MSG_SAUDACAO = (
    "Ola seja bem vindo a Personalizei, obrigado pela compra.\n"
    "Antes do envio de qualquer imagem e de extrema importancia que voce nos envie primeiro "
    "o numero do pedido, esse numero e o numero que esta depois das letras ID: "
    "Voce precisa digitar o numero ou copiar e colar, nao envie print pois o nosso sistema "
    "nao reconhece print."
)
MSG_PEDIR_PEDIDO = (
    "Por favor envie o numero do pedido, esse numero vem logo depois das letras ID:, "
    "voce pode encontrar esse numero no seu historico de pedidos, "
    "sem esse numero nao conseguimos te identificar."
)
MSG_FINALIZAR = (
    "Perfeito, seu pedido ja esta sendo preparado e sera enviado no menor tempo possivel. "
...
MSG_PIX = (
    ...
    "Apos efetuar o pagamento pela chave PIX nos envie o comprovante por favor."
)

PRECOS_EXTRA = {
    "10X15": 1.00, "15X21": 1.50, "Mini foto": 1.00,
    "A4": 3.00, "Ima": 2.50, "Mini ima": 2.50,
    "Polaroide": 1.00, "Tirinha": 1.00, "Cartao de Visita": 1.00,
}
...
def _upload_imagem_drive(image_url, phone):
    try:
        ...
        print(f"[Drive] OK {filename} ({len(image_bytes)//1024}KB)")
        return drive_url
    except Exception as e:
...
def pedido_existe(numero):
    try:
        ...
        if ws is None:
            return False
        return numero.upper() in [v.strip().upper() for v in ws.col_values(1) if v.strip()]
    except Exception:
        return False
...
def salvar_pedido(numero_pedido, produto="", quantidade="", sku="",
    ...
    try:
        ...
        ws.append_row([numero_pedido, data, produto, quantidade, sku, cliente, prazo, telefone, status, obs])
...
def vincular_pedido(phone, numero_pedido):
    ...
    if limite > 0:
        ...
        enviar_mensagem(phone, f"Perfeito, pedido {numero_pedido} recebido!")
...
def verificar_gmail():
    ...
    try:
        ...
        try:
            ws = get_sheet("Pedidos")
            if ws:
                pedidos_na_planilha = set(v.strip().upper() for v in ws.col_values(1) if v.strip())
        except Exception as e:
        ...
        for eid in ids:
            ...
            try:
                ...
                if numero and numero.upper() not in pedidos_na_planilha:
                    ...
                    if not sku:
                        m_kit = re.search(r'(KIT\s+(?:ATE\s+)?\d+\s+FOTOS?)', corpo, re.IGNORECASE)
                        if m_kit:
